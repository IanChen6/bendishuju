...
from get_db import add_task, job_finish, cancelling_task
        ...
        try:
            if post_data['Type'][0] == "TAXDATA":
                ...
                if post_data['BatchID'] and post_data['BatchYear'] and post_data['BatchMonth'] and post_data[
                    ...
                    # logger.info(db)
                    add_task('main01', '1433', 'ACTCenter', batchid, batchyear, batchmonth, companyid, customerid, "TAXDATA", jobname,
                             jobparams)
                    logger.info("任务添加成功,开始爬取")
                    pdict = {"1": account, "2": pwd, "3": batchid, "4": batchyear, "5": batchmonth, "6": companyid,
                             "7": customerid, "8": 'main01', "9": '1433', "10": 'ACTCenter'}
            ...
            if post_data['Type'][0] == "CUSTOMERINVOICE":
                ...
                if post_data['BatchID'] and post_data[
                    ...
                    logger.info("添加任务到数据库")
                    # logger.info(db)
                    add_task('main01', '1433', 'ACTCenter', batchid, batchyear, batchmonth, companyid, customerid, "CUSTOMERINVOICE",
                             jobname, jobparams)
                    logger.info("任务添加成功,开始爬取")
            ...
            if post_data['Type'][0] == "TAXAPPLYNORMAL":
                ...
                if post_data['BatchID'] and post_data['BatchYear'] and post_data['BatchMonth'] and post_data[
                    ...
                    logger.info("添加任务到数据库")
                    # logger.info(db)
                    add_task('main01', '1433', 'ACTCenter', batchid, batchyear, batchmonth, companyid, customerid, "TAXAPPLYNORMAL",
                             jobname,
                             jobparams)
                    logger.info("任务添加成功,开始爬取")
                    pdict = {"1": user, "2": pwd, "3": batchid, "4": batchyear, "5": batchmonth, "6": companyid,
                             "7": customerid, "8": 'main01', "9": '1433', "10": 'ACTCenter', "11": fw1, "12": fw2, "13": hw1,
            ...
            if post_data['Type'][0] == "TAXINVOICE":
                ...
                if post_data['BatchID'] and post_data['BatchYear'] and post_data['BatchMonth'] and post_data[
                    ...
                    logger.info("添加任务到数据库")
                    # logger.info(db)
                    add_task('main01', '1433', 'ACTCenter', batchid, batchyear, batchmonth, companyid, customerid, "TAXINVOICE",
                             jobname,
                             jobparams)
                    logger.info("任务添加成功,开始爬取")
                    pdict = {"1": user, "2": pwd, "3": batchid,  "4": companyid, "5": customerid, "6": 'main01', "7": '1433', "8": 'ACTCenter'}
                    pjson = json.dumps(pdict)
        ...
        try:
            if post_data['Type'][0] == "TAXDATA":
                ...
                if post_data['BatchID'] and post_data['BatchYear'] and post_data['BatchMonth'] and post_data[
                    ...
                    # logger.info(db)
                    cancelling_task('main01', '1433', 'ACTCenter', batchid, batchyear, batchmonth, companyid, customerid, "TAXDATA",
                                    jobname, jobparams)
